...
def tuple_list_to_table(tuple_list, filename):
    """For writing out SDRF table, data are stored as list of lists with tuples as items,
    similar to an ordered dictionary but keys (first items in tuple) can be non-unique)."""

    # Extract headers from the first entry in the rows collection
    header, value_rows = split_tuple_list(tuple_list)
    # Write to file
    write_table(header, value_rows, filename, sep='\t')


def write_table(header, rows, filename, sep='\t'):
    """Write a list of lists into a table with given header row."""
    with codecs.open(filename, 'w', encoding='utf-8') as out:
        writer = csv.writer(out, delimiter=sep, lineterminator='\n')
